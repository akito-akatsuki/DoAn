...
export default function HomePage() {
  ...
  // ================= STORY INTERACTIONS HOOKS & HANDLERS =================
  useEffect(() => {
    if (activeStoryGroup && user?.id && activeStoryGroup.items.length > 0) {
      const currentStory = activeStoryGroup.items[currentStoryIndex];
      const storyId = currentStory.id;

      if (activeStoryGroup.user?.id !== user.id) {
        // Ghi nhận lượt xem
        supabase
          .from("story_views")
          .upsert(
        ...
        supabase
          ...
          .then(({ data }) => {
            ...
            setLikedStories((prev) => {
              ...
              if (isLiked) next.add(storyId);
              else next.delete(storyId);
              return next;
            });
          });
      } else {
        // Lấy danh sách lượt xem và lượt thích nếu là chủ story
    ...
    try {
      if (isLiked) {
        await supabase
          .from("story_likes")
          .delete()
          .match({ story_id: currentStory.id, user_id: user.id });
        setLikedStories((prev) => {
          const next = new Set(prev);
  ...
  const handleReplyStory = async () => {
    ...
    try {
      const receiverId = activeStoryGroup.user.id;
      const convId = await getOrCreateConversation(user.id, receiverId);

      const storyPreviewUrl =
        currentStory.media_type !== "text" ? currentStory.media_url : null;
      const content = `Đã trả lời tin của bạn:\n\n"${textToSend}"`;

      await sendMessage(convId, user.id, content, storyPreviewUrl);
    } catch (e) {
      console.error(e);
                ...
                {storyInteractions.likes.length > 0 && (
                  <div>
                    <h4 className="font-bold text-sm text-muted-foreground uppercase tracking-wider mb-3 flex items-center gap-2">
                      ...
                      Đã thả tim ({storyInteractions.likes.length})
                    </h4>
                    <div className="flex gap-4 overflow-x-auto pb-2 snap-x [&::-webkit-scrollbar]:hidden">
